...
Rem
Rem    NAME
Rem      apex_embedded_ebs_setup.sql
Rem
Rem    DESCRIPTION
...
Rem    Example:
Rem
Rem      sqlplus apps/apps@<connect string> @apex_ebs_setup.sql
Rem
Rem
...
Rem      sbastida  10/21/2024 - Modified, removed custom schema creation and editions enablement with New EBS introduced in 12.2.14+
...
grant read on xx_apex_ebs_user to ^APEX;
grant read on fnd_responsibility_vl to ^APEX;
...
prompt .
  ...
  CREATE OR REPLACE FORCE EDITIONABLE VIEW "APPS"."XX_CUSTOMER_LOCATIONS" ("CUSTOMER_NUMBER", "CUSTOMER_NAME", "SITE_NUMBER", "ADDRESS1", "ADDRESS2", "ADDRESS3", "ADDRESS4", "CITY", "POSTAL_CODE", "STATE", "COUNTRY", "BILL_TO_LOCATION", "SHIP_TO_LOCATION", "LONGITUDE", "LATITUDE") AS 
...
--                hl.geometry
                hl.geometry.SDO_POINT.X AS longitude,
                hl.geometry.SDO_POINT.Y AS latitude
           FROM hz_parties hp,
                hz_party_sites hps,
